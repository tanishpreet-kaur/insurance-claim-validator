...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ result }) => {
  // Strong typing for verdict config
  ...
  const PolicyCheckItem: React.FC<{ label: string; value: boolean }> = ({
    label,
    value,
  }) => (
  ...
  return (
    <div className="w-full space-y-6 animate-fade-in">
      ...
      <div className={`flex items-center p-4 rounded-lg ${currentVerdict.bgColor}`}>
        {currentVerdict.icon}
        <h2 className={`ml-3 text-2xl font-bold ${currentVerdict.textColor}`}>
      ...
      <div className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-lg">
        ...
        <ul className="space-y-2">
          <PolicyCheckItem
            label="Policy is active and in good standing"
            value={result.policyChecks.policyActive}
          />
          <PolicyCheckItem
            label="Incident type is covered by policy"
            value={result.policyChecks.incidentCovered}
          />
          <PolicyCheckItem
            label="Claim filed within allowed time limit"
            value={result.policyChecks.timeLimitOk}
          />
        </ul>
      </div>
    </div>
